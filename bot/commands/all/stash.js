...
module.exports = {
    desc: "Store an item in your house",
    aliases: ['st'],
    usage: "[item] <amount>",
    cooldown: 5,
    guildOnly: true,
    ...
    task(Ai, msg, suffix){
        ...
        if(options.length>=0){
            ...
            if(amount == 0)return Ai.createMessage(msg.channel.id,`<@${msg.author.id}>, You can not store 0 of an item.`).catch(err => {handleError(Ai, __filename, msg.channel, err)});
            if(amount % 1 != 0)return Ai.createMessage(msg.channel.id,`<@${msg.author.id}>, You can only store whole numbers of items.`).catch(err => {handleError(Ai, __filename, msg.channel, err)});
